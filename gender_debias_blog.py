...
###csv work (x and y)
d = pd.read_csv("/content/drive/My Drive/Colab Notebooks/gender.csv",encoding="latin1")
df = pd.DataFrame(data=d)
df = df.fillna(method='ffill')
df=df[:30000]
x = df['sentence'].to_numpy()
#df.sentid = [1 if each =="female" else 0 for each in df.sentid]
y = df['sentid'].to_numpy()
...
def gender_subspace(pairs, embedding, num_components=5):
    mat = []
    print(pairs)
    for x, y in pairs:
